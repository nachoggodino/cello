...
export function validateEditorDocumentCommand(workbook: EditorWorkbook, command: EditorDocumentCommand): string | undefined {
  switch (command.type) {
    case "batch":
      return validateBatch(workbook, command.commands);
    case "update-cell":
      return (
        validateAddress(workbook, command.address) ??
        validateString(command.source, "Cell source") ??
        (["source", "content", "raw"].includes(command.mode) ? undefined : "Cell update mode is invalid.")
      );
    case "update-default":
    case "update-header":
      return validateSheetAndColumn(workbook, command.sheetIndex, command.colIndex) ?? validateString(command.source, "Cell source");
    ...
    case "toggle-modifier":
      return validateTarget(workbook, command.target) ?? (["bold", "italic", "strike"].includes(command.key) ? undefined : "Toggle modifier is invalid.");
    case "set-color":
      return (
        validateTarget(workbook, command.target) ??
        (["bg", "color"].includes(command.key) ? undefined : "Color modifier is invalid.") ??
        validateNonEmptyString(command.value, "Color value")
      );
    case "set-tone":
      return validateTarget(workbook, command.target) ?? (TEXT_TONES.includes(command.value) ? undefined : "Tone value is invalid.");
    case "set-sheet-columns":
      return validateSheet(workbook, command.sheetIndex) ?? (isOneOf(command.mode, [undefined, "normal", "fit"]) ? undefined : "Sheet columns mode is invalid.");
    case "set-sheet-rows":
      return validateSheet(workbook, command.sheetIndex) ?? (isOneOf(command.mode, [undefined, "wrap", "ellipsis"]) ? undefined : "Sheet rows mode is invalid.");
    case "toggle-column-fit":
    case "set-column-width":
      return (
        validateSheetAndColumn(workbook, command.sheetIndex, command.colIndex) ??
        (command.type === "set-column-width" && command.value !== undefined ? validateNonEmptyString(command.value, "Column width") : undefined)
      );
    case "toggle-row-wrap":
    case "set-row-height":
      return (
        validateExistingRow(workbook, command.address) ??
        (command.type === "set-row-height" && command.value !== undefined ? validateNonEmptyString(command.value, "Row height") : undefined)
      );
    case "merge-cell":
      return (
        validateExistingCell(workbook, command.address) ??
    ...
    case "add-row":
      return validateSheet(workbook, command.sheetIndex) ?? validateAfterIndex(command.afterRowIndex, workbook.sheets[command.sheetIndex]?.rows.length ?? 0, "row");
    case "add-column":
      return validateSheet(workbook, command.sheetIndex) ?? validateAfterIndex(command.afterColIndex, getVisibleColumnCount(workbook.sheets[command.sheetIndex]), "column");
    case "add-sheet":
      return undefined;
    case "remove-sheet":
      return validateSheet(workbook, command.sheetIndex) ?? (workbook.sheets.length > 1 ? undefined : "The last remaining sheet cannot be removed.");
    case "rename-sheet":
      return validateSheet(workbook, command.sheetIndex) ?? validateNonEmptyString(command.name, "Sheet name");
    case "clear-range":
      return validateRange(workbook, command.range) ?? (typeof command.includeModifiers === "boolean" ? undefined : "includeModifiers must be boolean.");
    case "fill-range":
...
function validateTarget(workbook: EditorWorkbook, target: EditorCommandTarget): string | undefined {
  if (target.scope === "column") {
    ...
    return target.colIndexes.find((index) => !isIndex(index)) === undefined ? undefined : "Column indexes must be non-negative integers.";
  }
...
function validateAddress(workbook: EditorWorkbook, address: CellAddress): string | undefined {
  return validateSheet(workbook, address.sheetIndex) ?? (isIndex(address.rowIndex) && isIndex(address.colIndex) ? undefined : "Cell coordinates must be non-negative integers.");
}
...
function validateExistingRow(workbook: EditorWorkbook, address: CellAddress): string | undefined {
  ...
  return address.rowIndex < (workbook.sheets[address.sheetIndex]?.rows.length ?? 0) ? undefined : "The targeted row does not exist.";
}
...
function validateExistingCell(workbook: EditorWorkbook, address: CellAddress): string | undefined {
  ...
  return address.colIndex < getVisibleColumnCount(workbook.sheets[address.sheetIndex]) ? undefined : "The targeted cell does not exist.";
}

function validateRange(workbook: EditorWorkbook, range: CellRange): string | undefined {
  return (
    validateSheet(workbook, range.sheetIndex) ??
    ([range.startRow, range.endRow, range.startCol, range.endCol].every(isIndex) && range.startRow <= range.endRow && range.startCol <= range.endCol
      ? undefined
...
function validateSheetAndColumn(workbook: EditorWorkbook, sheetIndex: number, colIndex: number): string | undefined {
  return validateSheet(workbook, sheetIndex) ?? (isIndex(colIndex) ? undefined : "Column index must be a non-negative integer.");
}

function validateSheet(workbook: EditorWorkbook, sheetIndex: number): string | undefined {
  return isIndex(sheetIndex) && workbook.sheets[sheetIndex] ? undefined : "The targeted sheet does not exist.";
}
...
function validateAfterIndex(index: number | undefined, length: number, label: string): string | undefined {
  ...
  return isIndex(index) && index < length ? undefined : `The insertion ${label} index does not exist.`;
}
