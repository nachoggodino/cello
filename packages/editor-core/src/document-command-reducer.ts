...
export function reduceEditorDocumentCommand(workbook: EditorWorkbook, command: EditorDocumentCommand): EditorWorkbook {
  switch (command.type) {
    case "update-cell":
    ...
    case "update-header": {
      ...
      return updateCellContentSource(
        resolution.workbook,
        {
...
function reduceCellUpdate(workbook: EditorWorkbook, command: Extract<EditorDocumentCommand, { type: "update-cell" }>): EditorWorkbook {
  if (command.mode === "raw") {
    return updateCellRaw(workbook, command.address, command.source);
  }
  return command.mode === "content" ? updateCellContentSource(workbook, command.address, command.source) : updateCellSource(workbook, command.address, command.source);
}
...
function updateTarget(workbook: EditorWorkbook, target: EditorCommandTarget, reducers: TargetReducers): EditorWorkbook {
  ...
  return target.colIndexes.reduce(
    (current, colIndex) =>
      reducers.column(current, {
...
function updateTargetModifiers(workbook: EditorWorkbook, target: EditorCommandTarget, source: string): EditorWorkbook {
  return updateTarget(workbook, target, {
    ...
    column: (current, address) => updateColumnModifierSource(current, address.sheetIndex, address.rowIndex, address.colIndex, source)
  });
}
