    ...
    <section className={shellClassName} data-revision={snapshot.revision}>
      {showToolbar ? (
        <div className="celloSourceToolbar" role="toolbar" aria-label="Source formatting and history">
          <button
            type="button"
            disabled={readOnly || !snapshot.histories.source.canUndo}
            onClick={undo}
          >
            Undo
          </button>
          <button
            type="button"
            disabled={readOnly || !snapshot.histories.source.canRedo}
            onClick={redo}
          >
            Redo
          </button>
          <button type="button" disabled={readOnly} onClick={() => { session.format("compact"); }}>
            Compact
          </button>
          <button type="button" disabled={readOnly} onClick={() => { session.format("pretty"); }}>
            Pretty
          </button>
        </div>
      ) : null}
      <Suspense fallback={<div className="celloSourceLoading" role="status">Loading source editor…</div>}>
        <CodeMirrorSourceSurface
          ariaLabel={ariaLabel}
          onBlur={() => { typingGroupRef.current.lastInputAt = 0; }}
