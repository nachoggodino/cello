...
export function sheetLayoutToModifiers(layout: SheetLayout | undefined): Array<{ raw: string }> {
  return [layout?.columns ? { raw: `columns:${layout.columns}` } : undefined, layout?.rows ? { raw: `rows:${layout.rows}` } : undefined].filter(
    (modifier): modifier is { raw: string } => Boolean(modifier)
  );
}
